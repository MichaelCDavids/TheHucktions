...
public class ManagementQueries {
    ...
    public List<Player> getAllPlayers() {
        return jdbi.withHandle((h) -> h.createQuery("select first_name, last_name, email, age, pos, weight, height from player")
    ...
    public boolean insertPlayerRecord(Player player) {
        jdbi.useHandle((h) -> h.execute("insert into player( first_name, last_name, email, age, pos, weight, height) values (?,?,?,?,?,?,?)",
                player.getFirstName(),
                player.getLastName(),
                player.getEmail(),
                player.getAge(),
